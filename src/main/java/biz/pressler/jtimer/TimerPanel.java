...
class TimerPanel extends JPanel {
    ...
    private TimerPanel() {
        ...
        try {
            InputStream file = this.getClass().getResource("/icons/" + icons.getString("icon")).openStream();
...
//		setOpaque(false);
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2D.drawImage(image, -10, 10, image.getWidth(), image.getHeight(), this);

        g2D.setFont(f);
        g2D.setColor(Color.gray);
        g2D.drawString("jTimer", 170, 33);
        g2D.setColor(Color.lightGray);
        g2D.drawString("jTimer", 167, 30);

        if (ssb.isStop()) {
            g2D.setColor(Color.darkGray);
            g2D.drawString(t, 150, 75);
        } else {
            g2D.setColor(Color.white);
            g2D.drawString(t, 150, 75);
        }
        g2D.setColor(Color.gray);
        g2D.fillRect(150, 85, percent * 120 / 100, 10);
        g2D.drawRect(150, 85, 120, 10);
        g2D.setColor(Color.lightGray);
        f = new Font("Arial", Font.BOLD, 14);
        g2D.setFont(f);
        g2D.drawString("H", 90, 147);
        g2D.drawString("M", 130, 147);
        g2D.drawString("S", 170, 147);

        f = new Font("Arial", Font.PLAIN, 12);
        g2D.setFont(f);
        g2D.drawLine(0, 152, 300, 152);
        g2D.drawString(infostr, 5, 165);
